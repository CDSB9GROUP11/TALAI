...
def setup_astra_session(
    bundle_path='secure-connect-resumes-database.zip',
    keyspace='embeddings',
    max_retries=3,
    retry_delay=5.
):
...
def create_resume_table(session):
...
#     logger.info(f"Found IDs: {found_ids}")
...
def fetch_existing_resumes(session, file_ids):
    all_rows = []
    if len(file_ids) < 15:
        logger.info("No chunking is required")
        placeholders = ",".join(["%s"] * len(file_ids))

        # Bulk query using IN clause
        select_query = f"""
            SELECT resume_id, embedding_vectors, metadata, section_text
            FROM embeddings.section_embed
            WHERE resume_id IN ({placeholders})
        """

        try:
            result = session.execute(select_query, tuple(file_ids))
            all_rows = result.all()
        except Exception as e:
            logger.error(f"Bulk query failed: {e}")
            return None, file_ids  # Assume all missing if query fails
    else:
        for chunk in chunk_list(file_ids, 15):
            logger.info(f"Enter fetching existing resumes for chunk: {chunk}")
            ...
            """
            try:
                result = session.execute(query, tuple(chunk))
...
def insert_resume_data(session, df):
